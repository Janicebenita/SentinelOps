...
<p align="center">
  ...
  &nbsp;•&nbsp;
  <a href="demo_storytelling_video.mp4"><strong>▶ Watch Enhanced Demo Video</strong></a>
  &nbsp;•&nbsp;
...
The finale extends—not replaces—the evidence-first workflow with an immutable, executable incident twin. Every repair candidate is evaluated against the same source commit, configuration snapshot, input fixtures, random seed, dependency fingerprint, network policy, resource limits, allowlist, protected paths, and linked evidence.

### Championship workflow

1. Reproduce TN + `SAVE10` three times in the network-disabled twin.
2. Compare three bounded candidates in a transparent Repair Tournament.
3. Disqualify any candidate that fails regression, unit, integration, Ruff, MyPy, or Bandit.
4. Exercise eight nearby conditions in the Counterfactual Lab.
5. Estimate behavioral blast radius from modified and transitive components, coverage, contracts, configuration scope, and critical paths.
6. Persist an adversarial review that challenges symptom masking, contract drift, insecure defaults, test weakening, and nearby-input failures.
7. Recommend Candidate C only after deterministic gates reject A as a false fix and B as a broader contract change.
8. Require a human decision and export a SHA-256 tamper-evident audit package. Nothing is automatically deployed.
...
### One-click AI storytelling video

Generate the complete 1920×1080, 30 FPS, H.264/AAC presentation with synchronized
Indian-English female narration and subtitles:

```powershell
.\.venv\Scripts\python.exe scripts\generate_storytelling_video.py --install
```

Subsequent runs can omit `--install`. The generator produces
`demo_storytelling_video.mp4`, `demo_storytelling_video.srt`, and `voice.wav` in the
repository root. Its canonical 5 minute 20 second timeline covers the command center,
incident, evidence, hypotheses, Digital Twin, deterministic replay, Repair Tournament,
counterfactual false-fix rejection, blast radius, adversarial review, human approval,
audit package, and reliability scorecard.
...
[**▶ Watch the complete narrated Reliability Digital Twin demo →**](demo_storytelling_video.mp4)
